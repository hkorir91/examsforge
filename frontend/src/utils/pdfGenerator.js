...
export async function generateExamPDF(exam, meta) {
  ...
  // Sanitizes unsupported characters (Greek letters, special symbols) that
  // crash jsPDF's default Helvetica font and cause spaced-out garbled text.
  const sanitizeText = (str) => {
    if (!str) return ''
    return str
      .replace(/π/g, '(pi)')
      .replace(/Π/g, '(Pi)')
      .replace(/θ/g, 'θ')       // theta — often shows as ¸ in some encodings
      .replace(/¸/g, 'θ')       // fix garbled theta from some AI responses
      .replace(/À/g, '(pi)')    // fix garbled pi from some AI responses
      .replace(/÷/g, '÷')
      .replace(/×/g, 'x')
      .replace(/≠/g, '≠')
      .replace(/≤/g, '<=')
      ...
      .replace(/¼/g, '1/4')
      .replace(/¾/g, '3/4')
      .replace(/°/g, '\u00b0')  // degree sign — usually safe in Helvetica
      .replace(/[^\x00-\xFF]/g, '?')  // replace any remaining non-Latin-1 with ?
  }

  const renderQuestionText = (text, indentLeft = margin) => {
    const maxW = pageW - margin - indentLeft
    const { before, table, after } = splitTextAroundTable(sanitizeText(text || ''))

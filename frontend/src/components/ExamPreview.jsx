...
// How many questions to show as free preview per section
...
export default function ExamPreview({ exam, meta, examId, onRegenerate }) {
  ...
  // ── Render a section (question paper or scheme) ──────
  const renderSection = ({ key, label, sublabel, type }) => {
      ...
      <div key={key} className="mb-8">
        ...
        <div className="space-y-3">
          {section.questions.map((q, idx) => {
            // Free users only see first FREE_PREVIEW_COUNT questions clearly;
            // the rest are blurred with an upgrade CTA overlay
            const isLocked = !isPremium && idx >= FREE_PREVIEW_COUNT
            const isEditing = editingQ?.section === key && editingQ?.qNum === q.num

            return (
              <div key={q.num} className={`relative border rounded-xl transition-all group ${isLocked ? 'border-gray-100' : 'border-gray-100 hover:border-gray-200'}`}>
                ...
                {/* Lock overlay — shown over blurred questions */}
                {isLocked && (
                  <div className="absolute inset-0 flex items-center justify-center rounded-xl bg-white/70">
                    <Link
                      to="/pricing"
                      className="flex items-center gap-2 bg-brand-blue text-white text-xs font-bold px-4 py-2 rounded-full shadow-md hover:bg-brand-blue-dark transition-colors no-underline"
                    >
                      <svg width="12" height="12" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
                        <path d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/>
                      </svg>
                      Unlock Full Exam — Upgrade
                    </Link>
                  </div>
                )}
              </div>
        ...
        {/* Banner below the last free question if locked */}
        {!isPremium && section.questions.length > FREE_PREVIEW_COUNT && (
          <div className="mt-4 flex items-center gap-3 bg-amber-50 border border-amber-200 rounded-xl p-4 no-print">
            <span className="text-lg">🔒</span>
            <div className="flex-1">
              <p className="text-xs font-bold text-amber-800">
                {section.questions.length - FREE_PREVIEW_COUNT} more questions hidden
              </p>
              <p className="text-xs text-amber-600 mt-0.5">
                Upgrade to Premium to view the full exam paper and download as PDF.
              </p>
            </div>
            <Link
              to="/pricing"
              className="text-xs font-bold bg-amber-500 text-white px-4 py-2 rounded-full hover:bg-amber-600 transition-colors no-underline flex-shrink-0"
            >
              See Plans →
            </Link>
          </div>
  ...
  // ── Render marking scheme (also partially locked for free) ──
  const renderScheme = () => (
    <div className="mt-3 bg-blue-50 border border-blue-200 rounded-xl p-5">
          ...
          <div key={sec} className="mb-5">
            ...
            <div className="space-y-2">
              {section.questions.map((q, idx) => {
                const isLocked = !isPremium && idx >= FREE_PREVIEW_COUNT
                return (
                  <div key={q.num} className="relative bg-white rounded-xl p-3 border border-blue-100">
      ...
      {!isPremium && (
        <div className="mt-3 bg-blue-100 border border-blue-300 rounded-xl p-4 text-center">
          <p className="text-xs font-bold text-blue-800 mb-2">Full marking scheme available on Premium</p>
          <Link to="/pricing" className="btn-primary text-xs py-2 px-5 no-underline inline-block">
            Upgrade Now →
          </Link>
        </div>
      )}
    ...
    <div className="h-full overflow-y-auto bg-gray-50">
      ...
      <div className="sticky top-0 z-10 bg-white border-b border-gray-100 px-5 py-3 flex items-center justify-between flex-wrap gap-3 no-print">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Download button — always visible, routes free users to pricing */}
          <button
            onClick={handleDownloadPDF}
            className={`text-xs py-2 px-3 flex items-center gap-1.5 font-semibold rounded-xl border transition-all ${
              isPremium
                ? 'btn-primary'
      ...
      <div className="max-w-3xl mx-auto p-6">
        <div className="exam-paper bg-white rounded-2xl shadow-sm border border-gray-100 p-10">
          ...
          {/* Free-user preview notice */}
          {!isPremium && (
            <div className="mb-6 flex items-start gap-3 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-xl p-4 no-print">
              <span className="text-xl mt-0.5">👁️</span>
              <div>
                <p className="text-sm font-bold text-blue-900">You're viewing a free preview</p>
                <p className="text-xs text-blue-700 mt-0.5">
                  The first {FREE_PREVIEW_COUNT} questions per section are visible. Upgrade to access the full exam, complete marking scheme, and PDF download.
                </p>
              </div>
              <Link to="/pricing" className="flex-shrink-0 text-xs font-bold bg-brand-blue text-white px-4 py-2 rounded-full hover:bg-brand-blue-dark transition-colors no-underline">
                Upgrade →
              </Link>
            </div>
          )}
          ...
          >
            <span>📋 Marking Scheme {!isPremium && <span className="text-xs text-gray-400 font-normal">(preview only)</span>}</span>
            <svg

...
// Geography: water_cycle, rock_cycle, fold_diagram, composite_volcano, contour_map,
//            earthquake_waves, drainage_pattern, population_pyramid
//
// If diagram.svg is present (AI-generated), it is rendered directly.
// Static components are the fallback for web preview and PDF placeholders.

export default function DiagramRenderer({ diagram }) {
  if (!diagram || !diagram.type) return null

  // ── AI-generated SVG path ─────────────────────────────────────────────────
  // When the backend successfully generates an SVG for this diagram, it stores
  // it in diagram.svg. Render it directly — this is the highest quality path.
  if (diagram.svg) {
    const caption = diagram.caption || ''
    return (
      <div className="flex flex-col items-center my-3">
        <div
          className="border border-gray-200 rounded-lg bg-white p-2 inline-block max-w-full overflow-hidden"
          style={{ lineHeight: 0 }}
          // SVG is generated by Claude with no scripts — safe to render inline.
          // The generateSingleSVG function already strips <script> and javascript: URIs.
          dangerouslySetInnerHTML={{ __html: diagram.svg }}
        />
        {caption && (
          <p className="text-xs text-gray-500 mt-1.5 italic text-center">{caption}</p>
        )}
      </div>
    )
  }
  ...
  const components = {
    ...
    shield_volcano: CompositeVolcano,
